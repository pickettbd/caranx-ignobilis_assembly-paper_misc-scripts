...
class Scaffold:
	...
	def __init__(self, name='', components=[]):
		...
		self.num_bases_unknown = 0
		self.components = []
		#self.components = components # shallow copy
	...
	def getContigs(self):
		...
		for component in self.components:
			if isinstance(component, Contig):
				#contigs.append(component) # shallow copy
...
if __name__ == "__main__":
	...
	agp = pl.Path(agp_fn)
	tig = pl.Path(tig_fn)
	scaff = pl.Path(scaff_fn)

	# read the input files and create the scaffolds in memory
	scaffs = generateScaffoldsFromAgpAndContigFasta(agp, tig)

	# write the output
	with open(scaff, 'w') as ofd:
		for scaffold in scaffoldsSortedIterate(scaffs, sort_method, reverse):
			ofd.write(scaffold.getFasta())
